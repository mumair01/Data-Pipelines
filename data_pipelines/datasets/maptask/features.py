...
DEFAULT_POS_DELAY_TIME_MS = 100 # Assume that each POS calculation is delayed by 100ms.

################################# METHODS ####################################


def get_voice_activity_annotations(maptask_root_path,dialogue, participant,
        minimum_va_classification_time_ms=DEFAULT_MINIMUM_VA_CLASSIFICATION_TIME_MS,
        frame_step_size=DEFAULT_FRAME_STEP_SIZE,
        voice_activity_label=DEFAULT_VOICE_ACTIVITY_LABEL) -> pd.DataFrame:
    """
    ...
    va_times = [va_time for va_time in va_times if \
        va_time[1] - va_time[0] >= minimum_va_classification_time_ms/1000]
...
def get_pos_annotations_with_delay(
    ...
    pos_tags_to_idx = {
        ...
        tag : i+1 for i, tag in enumerate(POS_TAGS)
    }
